...
`Transaction ID` · `Date` · `Time` · `Shift` · `Transaction Type [Restock/Sale/Return]` · `Branch ID` · `Branch Name` · `City` · `Country` · `Employee ID` · `Employee Name` · `Product SKU` · `Category` · `Product Name` · `Color` · `Size` · `Quantity` · `Unit Cost` · `Unit Retail Price`
...
> `Transaction ID` · `Date` · `Time` · `Shift` · `Transaction Type` · `Branch ID` · `Employee ID` · `SKU` · `Quantity` · `Unit Cost` · `Unit Retail Price` · `Date(Year)` · `Date(Quarter)` · `Date(Month Index)` · `Date(Month)`
...
- Assumed to be working at fixed branches at fixed shifts; branch ID and shifts get computed automatically using XLOOKUP from employee sheet when employee ID is input
